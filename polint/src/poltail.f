...
C     These subroutines are based on Seonho's version of POLRAD (elastic part)
C     Hacked it slightly to handle both He-3 and proton. Also updated He-3 Form factors.
C-----------------------------------------------------------------------------------
      BLOCK DATA FOR_ELASTIC
...
C$$$  W2  = 0.938272*(0.938272 + 2.0*XNU) - Q2             
...
C$$$  W       = SQRT(W2)
C$$$  XJACOB2 = 2.0*EP*W/(2.0*0.938272*EP*1.0E-3+Q2)
...
C$$$  XJACOB  = 1.0                              ! TO COMPARE WITH THE ORIGINAL POLRAD20

      Y_ELAS  = 1.0/(1.0 + AMT/(2.0* E0*1.0E-3 * SIN(0.5*TH_RAD)**2) )

      
C$$$  W2_ELAS = AMH**2 - 2.0*(AMT - AMH)*Y_ELAS*E0*1.0E-3 ! USE PROTON MASS FOR W2
...
C
      ...
      IF(TARZ.EQ.1. .AND. TARA.EQ.1.) THEN ! CPROT
        ...
        SIGMA_RAD  = (SIB * EXTAI1 * (1.+ ALFA/PI *
     .   (DELTA-DELINF) ) + TAIL) * XJACOB ! C$$$
      ELSEIF(TARZ.EQ.2. .AND. TARA.EQ.3. ) THEN
        SIGMA_RAD  = (SIB * FACTOR1* (1.+ ALFA/PI *
     .   DEL_SUB) + TAIL) * XJACOB
      ELSE
        STOP "Problem 1"
      ENDIF
...
C-----------------------------------------------------------------------------------
     ...
     &     TARZ,FERMOM,AMM,AMN,CHBAR,BARN,S,X,SX,SXP,Y,YM,
     .      W2,ALS,ALX,ALM,ALY,SQLS,SQLX,SQLY,SQLM,ALLM,AN,
     .     TAMIN,TAMAX,XS,YS,TPL,TMI
...
*     ELASTIC R
*
C$$$  R_EL = ((AMT/AMH)*SX-Y)/(AMT/AMH + TAU)
      R_EL = (SX-Y)/(1.0 + TAU)
...
C-----------------------------------------------------------------------------------
     ...
     &     SQLS,SQLX,SQLY,SQLM,ALLM,AN,TAMIN,TAMAX,XS,YS,TPL,TMI
      ...
      REAL*8 AMP,AMP2,AP,AP2,AML,AML2,AL2,AMC2,AMH,AMT,TARA,
     &     TARZ,FERMOM,AMM,AMN,CHBAR,BARN,S,X,SX,SXP,Y,YM,W2,
     .      ALS,ALX,ALM,ALY,SQLS,SQLX,SQLY,SQLM,ALLM,AN,TAMIN,
     .      TAMAX,XS,YS,TPL,TMI
      ...
      DO I = 1,2
         TEMP  =  DGAUSS( TAIL_INT_OVER_R_EL, TCUT(I),
     .    TCUT(I+1), 1.0D-6 )
...
C
     ...
     .SQLS,SQLX,SQLY,SQLM,ALLM,AN,TAMIN,TAMAX,XS,YS,TPL,TMI
      ...
      DEL1  = -YM*(ALM*ALLM**2/2.+2.*FSPEN(2D0*SQLM/(Y+SQLM))
     .       -PI2/2.)/SQLM
...
C-----------------------------------------------------------------------------------
  ...
  10  SUML = SUML + 2.*(T+A2)*ALLMI/3. - 10./9. + 
     .   4.*A2*(1.-A2*ALLMI) /3. /T
...
****************** FSPEN **************************************
  ...
  1   IF (X-.5D0) 2,2,3
    ...
    3 IF (X-1D0 ) 4,4,5
    4 FSPEN = F1 -   DLOG(X) * DLOG( 1D0-X+1D-10  ) - RFSPENS(1D0-X)
      RETURN

    5 IF (X-2D0 ) 6,6,7
    6 FSPEN = F1 -.5*DLOG(X) * DLOG( (X-1D0)**2/X ) + RFSPENS(1D0-1D0/X)
      RETURN

    7 FSPEN = 2D0*F1-.5D0*DLOG(X)**2-RFSPENS(1D0/X)
      RETURN

    8 IF (X+1D0) 10,9,9
   9  FSPEN =-.5D0 *DLOG(1D0-X)**2-RFSPENS( X/(X-1D0) )
      RETURN

  10  FSPEN =-.5* DLOG(1.-X)*DLOG(X**2/(1D0-X))-F1+
     .   RFSPENS(1D0/(1D0-X))
...
****************** TAILS **************************************
     ...
     .       * BB*SPE + 8.*B1I*SPE**2 + 2.*BI12*CCPE*SPE*TA**2)/4.
      ...
      EEB    = ((CCPE - SPE*TA)**2*BB + 4.*(CCPE - SPE*TA)*
     .           B1I*SPE + 4.*B11I*SPE**2)/4.

      CALL FFU(1,BB ,BIS ,BIR ,BI12 ,BI1PI2,SIR,SIS,SI12,EIS,
     .       EIR ,EI12 ,EI1PI2,TA)
      CALL FFU(2,EB ,EIS ,EIR ,EI12 ,EI1PI2,OIR,OIS,OI12,EEIS,
     .       EEIR,EEI12,EEI1I2,TA)
      CALL FFU(3,EEB,EEIS,EEIR,EEI12,EEI1I2,0D0,0D0,0D0,0D0,
     .       0D0,0D0  ,0D0   ,TA)
...
*
      ...
      END
C-----------------------------------------------------------------------------------
CSL      SUBROUTINE FFHE3(T,GE,GM)
CSL      IMPLICIT REAL*8(A-H,O-Z)
CSL      COMMON/CMP/AMP,AMP2,AP,AP2,AML,AML2,AL2,AMC2,AMH,
CSL     .FERMOM,AMM,AMN,CHBAR,BARN,ISF20
C        COMMON/POLTARG/AMT,TARA,TARZ
CSL      TF=T/CHBAR**2
CSL      IF (TF.GE.0.0) THEN
CSL         QF=SQRT(TF)
CSL      ELSE
CSL         QF=-SQRT(-TF)
CSL      ENDIF
...
CSL      P=.9
CSL      Q0=3.98
CSL      F0=DDEXP(-A**2*TF) - B**2*QF**2*DDEXP(-C**2*TF)
CSL      FM=DDEXP(-AM**2*TF) - BM**2*QF**2*DDEXP(-CM**2*TF)
CSL      DF=D*DDEXP(-((QF-Q0)/P)**2)
CSL      GE=(F0+DF)*TARZ
CSL      GM=FM*TARA      * (-2.13)
CSL      END
